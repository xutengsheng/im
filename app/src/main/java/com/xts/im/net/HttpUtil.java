...
import com.xts.im.util.SystemUtil;
...
public class HttpUtil {
    ...
    private volatile ZhihuService mApiService;
...
/*    private Retrofit getRetrofit(OkHttpClient okHttpClient) {
    ...
    public class LoggingInterceptor implements Interceptor {
        ...
        public Response intercept(Chain chain) throws IOException {
            ...
            Log.i("Data:", response.peekBody(4096).string());
            return response;
        }
    }

    volatile ZhihuService mZhihuApi;
    //创建Retrofit请求数据接口,以前这么写是因为baseurl不固定
    public ZhihuService getZHihuApi(){
        if (mZhihuApi == null){
            synchronized (HttpUtil.class){
                if (mZhihuApi == null){
                    mZhihuApi = mRetrofitBuilder.baseUrl(ZhihuService.sZhiHuUrl)
                            .build()
    ...
    //创建Retrofit请求数据接口,以前这么写是因为baseurl不固定
    public WanService getWanApi(){
        if (mWanService == null){
            synchronized (HttpUtil.class){
                if (mWanService == null){
                    mWanService = mRetrofitBuilder.baseUrl(WanService.sWanUrl)
                            .build()
   ...
   /* public ZhihuService getApiService() {
        if (mApiService == null) {
            synchronized (HttpUtil.class) {
                if (mApiService == null) {
                    mApiService = mRetrofit.create(ZhihuService.class);
                }
            }
        }
        return mApiService;
